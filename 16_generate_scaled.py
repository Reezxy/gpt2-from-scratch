...
# Must match the training config from step 15 exactly
block_size = 256
model = GPT2(
...
model.load_state_dict(torch.load("gpt2_scaled.pt", map_location=device))
...
for prompt in prompts:
    result = generate(model, tokenizer, prompt, max_new_tokens=120, block_size=block_size, device=device, temperature=0.8)
    print("=" * 60)
    print(result)
    print()