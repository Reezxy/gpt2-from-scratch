...
# ---------- Model (same as before, with the proper GPT-2 init) ----------
...
class MultiHeadAttention(nn.Module):
    def __init__(self, embed_dim, num_heads):
        ...
        self.out_proj.RESIDUAL_SCALE = True
...
# ---------- Dataset: several books from Project Gutenberg ----------
...
if not os.path.exists(corpus_path):
    ...
    for book_id, title in BOOKS.items():
        ...
        # Roughly strip the Gutenberg boilerplate at the start and end
        start_marker = "*** START"
        end_marker = "*** END"
    ...
    with open(corpus_path, "w", encoding="utf-8") as f:
        f.write(full_text)
    print(f"\nCorpus assembled: {len(full_text):,} characters")
else:
    with open(corpus_path, "r", encoding="utf-8") as f:
        full_text = f.read()
    print(f"Corpus already present: {len(full_text):,} characters")

tokenizer = tiktoken.get_encoding("gpt2")
print("Tokenising (this can take a moment)...")
...
# ---------- Training with checkpointing (Ctrl+C is safe at any time) ----------

batch_size = 24
num_steps = 3000
eval_interval = 100
checkpoint_path = "gpt2_scaled.pt"
...
try:
    for step in range(num_steps):
        ...
        if step % eval_interval == 0 or step == num_steps - 1:
            ...
            print(f"Step {step:5d} | train loss {loss.item():.4f} | val loss {val_loss.item():.4f} | {elapsed/60:.1f} min")

            torch.save(model.state_dict(), checkpoint_path)

except KeyboardInterrupt:
    print("\nInterrupted with Ctrl+C - saving the latest state...")
    torch.save(model.state_dict(), checkpoint_path)

print(f"\nModel saved as {checkpoint_path}")